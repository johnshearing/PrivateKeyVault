...
A huge problem is that the private key generated by the device is a central point of attack. The end user has no absolute assurance that the algorithm used to generate the key cannot be hacked and that the private key cannot be predicted. So if some individual or government were to put a back door into the algorithm then everyone loses individual control of their accounts all at the same time. Avoiding this kind of disaster is the whole point decentralization using blockchains. Individuals must have the right to generate their own private keys using a method known to be completely random. The private key is how an account is controlled. How can we hand this control over to any single entity?  
